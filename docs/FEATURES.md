...
| **Company + people search** | Faceted Elasticsearch queries (industry/location/tech stack; seniority/department for people), hydrated from Postgres, emails masked unless revealed. | `searchService.js`, `pages/People.jsx`, `pages/Companies.jsx` |
| **Credit-gated email reveal** | Reserve → find (deterministic `first.last@domain` pattern) → optional Hunter.io verify → commit/refund. Idempotency-key protected. | `revealService.js`, `emailFinderService.js`, `emailVerifierService.js`, `creditService.js` |
...
| **Lists** | Save contacts/companies into named lists; CSV export (credit-gated). | `listService.js`, `pages/Lists.jsx`, `pages/ListDetail.jsx` |
| **Sequences (multi-step email cadences)** | `EMAIL`/`WAIT` steps, enroll/pause/resume/unenroll, a 60-second BullMQ tick advances due enrollments, funnel + per-step analytics. Gated to paid plans. | `sequenceService.js`, `jobs/processors/sequenceProcessor.js`, `pages/SequenceBuilder.jsx`, `pages/SequenceDetail.jsx` |
...
| **Support/sales ticketing (tenant side)** | User creates a ticket, replies to it; status flips `UNANSWERED`⇄`ANSWERED` as each side replies. | `ticketService.js`, `pages/Tickets.jsx`, `pages/TicketDetail.jsx` |
| **Support/sales ticketing (admin side)** | Separate `/control` panel: reply, close, live unread-ticket polling. | `adminTicketController`, `pages/admin/AdminTickets.jsx`, `pages/admin/AdminTicketDetail.jsx` |
...
| **GDPR/CCPA data-subject opt-out & erasure** | Unauthenticated `POST /privacy/opt-out` immediately redacts matching contacts and registers the email against future guessed reveals. | `privacyService.js` |
...
| **Admin action audit log** | Every suspend/unsuspend/plan-change/add-credits records which super admin did it, when, and (for plan/credits) the before→after values. Surfaced at `/control/audit-log`. | `AdminAuditLog` model, `adminService.recordAuditLog`, `pages/admin/AdminAuditLog.jsx` |
...
| **Team/seat invites** | Not built — every registration creates a brand-new workspace; `Role`/`Membership` support multi-seat, but there's no invite flow to actually add a second person to one. Open P0 item in `TODO.md`. |
