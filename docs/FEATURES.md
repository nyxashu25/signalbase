...
| **Chrome extension** | **Built (2026-08-24)** — `extension/` (Manifest V3, load-unpacked). On any `linkedin.com/in/…` profile it calls `POST /extension/observe` (API-key auth, keys minted in Settings → API & Extension): found contacts reveal email+phone for 4 credits (`EXTENSION_REVEAL`); unknown profiles queue as `MissingPerson` (admin "Pending peoples"); title mismatches queue as `LostChild` (admin "Childs found", Apply updates the shared contact). Not yet on the Chrome Web Store — distributed as a zipped build. |
