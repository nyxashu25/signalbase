...
describe('marketing: Pricing', () => {
  it('shows all four tiers with monthly per-seat prices', () => {
    mockFetchRoutes([]);
    ...
    // Organization's seat floor is stated right on the card.
    expect(screen.getAllByText(/min 3 seats/).length).toBeGreaterThan(0);
  });
  ...
  it('recomputes prices when the billing interval changes (annual = 12 months − 20%)', async () => {
    ...
    // 29 * 12 * 0.8 — the same math the backend charges (planPriceForInterval).
