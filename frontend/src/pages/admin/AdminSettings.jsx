...
export function AdminSettings() {
  const { data: settings, isLoading } = useGetAdminRazorpaySettingsQuery();
  const [save, { isLoading: saving, error }] = useSaveAdminRazorpaySettingsMutation();
  ...
  async function handleSubmit(e) {
    ...
    if (keySecret.trim()) body.keySecret = keySecret.trim();
    ...
    <div className="max-w-xl">
      ...
      <div className="mt-6 rounded-lg border border-white/10 bg-ink-900 p-6">
        <div className="flex items-center justify-between">
          <p className="text-base font-bold text-white">Razorpay</p>
          {!isLoading && (
            <span
        ...
        <p className="mt-1 text-xs text-ink-300">
          Once a key and secret are saved here, credit purchases across every workspace switch
          from simulated checkout to real Razorpay orders automatically — no deploy needed.
        </p>

        {settings?.configured && (
          <dl className="mt-4 grid grid-cols-2 gap-4 border-t border-white/10 pt-4">
            <div>
              <dt className="text-xs font-bold uppercase tracking-wide text-ink-300">Key ID</dt>
              <dd className="mt-1 font-mono text-sm text-white">{settings.keyId}</dd>
            </div>
            <div>
              <dt className="text-xs font-bold uppercase tracking-wide text-ink-300">Key secret</dt>
        ...
        <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3 border-t border-white/10 pt-6">
          <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-wide text-ink-300">
            ...
            <input
              ...
              placeholder={settings?.keyId ?? 'rzp_live_...'}
          ...
          <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-wide text-ink-300">
            ...
            <input
              ...
              placeholder={settings?.configured ? 'Leave blank to keep the saved secret' : 'Key secret'}
              className="h-10 rounded-md border border-white/15 bg-white/5 px-3 font-mono text-sm font-normal normal-case text-white outline-none focus:border-neon-violet"
            />
          </label>
          <label className="flex flex-col gap-1 text-xs font-bold uppercase tracking-wide text-ink-300">
            ...
            <input
              ...
              placeholder={
                settings?.hasWebhookSecret ? 'Leave blank to keep the saved secret' : 'Webhook secret'
              }
              className="h-10 rounded-md border border-white/15 bg-white/5 px-3 font-mono text-sm font-normal normal-case text-white outline-none focus:border-neon-violet"
            />
          </label>
